...
(function () {
  ...
  function belowList(b) {
    ...
    for (let d = Math.max(1, T); d <= Math.min(9, b - 1); d++) list.push(pre(d + "%"), pre(d + "."));
    for (let t = 1; t < T; t++) list.push(t === 1 ? `${pre(1)}:not(${pre(100)})` : pre(t));
    if (T >= 1) for (let v = 0; v < u; v++) list.push(T === 1 && v === 0 ? `${pre(10)}:not(${pre(100)})` : pre(`${T}${v}`));
    return list;
  }
  ...
  function alerts(ctx) {
    ...
    w.comment("数値が赤くなるとき（ココフォリアは最大値の80%以下で赤くします）");
