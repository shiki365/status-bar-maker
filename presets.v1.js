...
(function () {
  ...
  const ITEM_TYPES = [
    ["none", "なし"], ["gem", "宝石（ひびが入って砕ける）"], ["flower", "花（しおれて散る）"], ["moon", "月（欠けていく）"],
    ["star", "星（光が消えていく）"], ["candle", "ろうそく（短くなって消える）"], ["heart", "ハート（割れる）"],
  ];
  ...
  const DESIGNS = {
    ...
    crystal: {
      label: "砕ける宝石", desc: "減るほどバーにひびが入り、横の宝石・星・月が壊れていきます。削られていく緊張感を見せたい卓に。",
  ...
  window.BarPresets = {
    ...
    NAME_POS, NAME_STYLES, DECO_TYPES, ITEM_TYPES, DEFAULT_ITEMS, BASE_LOOK, DESIGNS, SAMPLE_STATUS, CHAR_COLORS,
  };
})();
